...
import os, random, time, shutil, humanize, string, math, sqlite3, re, traceback, sys
...
def dostuff(nbfiles, path=""):
    ...
    def manycirclestest(nbImages, fld):
        ...
        while nbImages > 0:
            nbImages -= 1
            full_path = fld + "/" + randname(9)
            full_path = circleImg(full_path, ext=".png", sizex=800, sizey=600)
            print("[DEBUG] created image...")
            size = os.path.getsize(full_path)
            print("        " + humanize.naturalsize(size) + " " + full_path)
            total += size
    ...
    print("Created " + str(created_files) + " with a total size of " + str(humanize.naturalsize(created_size)) + " in " + str(time.time()-ts))

if __name__ == "__main__":
    if int(sys.version[0]) < 3:
        print("You proably want to use python 3 because this uses unicode")
        sys.exit(-1)
    ...
    """
    print("Boting around")
    bo = Bot("/Users/thomas/Pydio/tests/seafile/Seafile/Ma Bibliothèque/")
    bo.dosomethings(NBFILES)
    print(bo)
